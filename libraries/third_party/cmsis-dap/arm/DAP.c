...
/* ExLink port: SWD 走 SPI 硬件引擎, 官方 Set_Clock_Delay 只设 bit-bang 延时, 对 SPI 无效。
 * 这里 extern 出 SPI 传输层的设频函数, 在 DAP_SWJ_Clock 里同步把频率设到 SPI 硬件。 */
extern void dap_swd_spi_set_clock(uint32_t hz);
...
static uint32_t DAP_SWJ_Clock(const uint8_t *request, uint8_t *response) {
  ...
  dap_swd_spi_set_clock(clock);   /* ExLink: 把 pyOCD 请求的频率真正设到 SPI 硬件 SWCLK */
